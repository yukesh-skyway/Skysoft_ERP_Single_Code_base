      ...
      for (const newDefect of newDefects) {
        try {
          ...
          vrlCreationResults.failed.push({
            external_defect_id: newDefect.external_defect_id,
            error: error.message
          });
...
for (const defect of existingDefects) {  // ← only existing defects
  ...
  if (defectId && !isNaN(defectId) && defectId > 0) {
    ...
    // Group repair items by RO
    const repairItemsByRO = {};
    for (const item of repairItems) {
      ...
      for (const defect of defects) {
        if (defectId && !isNaN(defectId) && defectId > 0 && defectId == defect.defect_id) {
          roId = defect.external_ro_id;
          break;
        } else if (smId && !isNaN(smId) && smId > 0 && smId == defect.sm_id) {
          roId = defect.external_ro_id;
          break;
        }
      }
      ...
      if (roId) {
        if (!repairItemsByRO[roId]) repairItemsByRO[roId] = [];
        repairItemsByRO[roId].push(item);
      }
    }
...
repairPORData.service_completion_date = formatUtcToMysql(serviceCompletionDate, null, true);
      ...
      if (repairItemsByRO[roId]) {
        for (const item of repairItemsByRO[roId]) {
          await processSingleRepairItem(item, roId, userId);
        }
      }
    }
 ...
 * POST /updateVehicleDefect
 * 
 * 🔴 NOTE (FIX #4): This v1 endpoint redirects to v2 for code simplification.
 * PHP v1 actively calls pushToMotive() without the isMotiveEnabled() flag check.
 * In the Node.js version, both v1 and v2 use the same logic in processSingleDefectV2(),
 * which calls pushToMotive() when defect_source === 'motive' AND isMotiveEnabled().
 * 
 * This is an acceptable behavioral difference because:
 * - v2 has the same logic as v1 in PHP
 * - The Motive push happens conditionally based on defect source
 * - Garages should be using v2 endpoint anyway
 */
async function updateVehicleDefect(req, res) {
  return updateVehicleDefect_v2(req, res);
}
...
async function processSingleRepairItem(item, externalRoId, userId, dbConn = db) {
  ...
  try {
    ...
    const mechanics = item.mechanic_assignments || []; // 🔴 BUG FIX #2: Correct array key (PHP line 1894)
    ...
    // 🔴 FIX 1: Declare allMechanicNotes OUTSIDE if blocks (used after both mechanics AND notes loops)
    const allMechanicNotes = [];

    // 2. Process mechanic assignments with ACTUAL schema (17 columns!)
    if (mechanics && mechanics.length > 0) {
      for (const mechanic of mechanics) {
        const mechanicName = mechanic.name || mechanic.mechanic_name || 'Unknown';
        const bayNumber = mechanic.bay_number || mechanic.bay || null;
        const mechanicStatus = mechanic.status || 'Assigned';
        const approvalStatus = mechanic.approval_status || 'Not_Submitted';
        const approvedBy = mechanic.approved_by || null;
        const approvedAt = mechanic.approved_at ? formatUtcToMysql(mechanic.approved_at) : null;
        const scheduledStart = mechanic.scheduled_start ? formatUtcToMysql(mechanic.scheduled_start) : null;
        const scheduledEnd = mechanic.scheduled_end ? formatUtcToMysql(mechanic.scheduled_end) : null;
        const actualStartDatetime = mechanic.actual_start_datetime || mechanic.actual_start ? formatUtcToMysql(mechanic.actual_start_datetime || mechanic.actual_start) : null;
        const actualEndDatetime = mechanic.actual_end_datetime || mechanic.actual_end ? formatUtcToMysql(mechanic.actual_end_datetime || mechanic.actual_end) : null;
        const mechanicChallengeNotes = mechanic.mechanic_challenge_notes || mechanic.challenge_notes || mechanic.notes || '';
        const invoiceNotes = mechanic.invoice_notes || '';
        const invoiceHours = mechanic.invoice_hours || 0;
        const actualHours = mechanic.actual_hours || mechanic.hours_worked || mechanic.hours || 0;
        const duration = mechanic.duration || 0;
        const garageMechanicId = mechanic.garage_mechanic_id || null;

        // 🔴 FIX 1: PHP ALWAYS pushes mechanic notes, even when challenge_notes is empty
        allMechanicNotes.push(`${actualEndDatetime} - ${mechanicName} (Challenge) ${mechanicChallengeNotes}`);

        // Check if mechanic assignment already exists
        if (garageMechanicId) {
          const [existingAssignment] = await dbConn.query(
            'SELECT id FROM mechanic_assignments WHERE repair_item_id = ? AND mechanic_name = ? AND ((scheduled_start IS NULL AND ? IS NULL) OR scheduled_start = ?)',
            [repairItemId, mechanicName, scheduledStart, scheduledStart]
          );

          if (existingAssignment && existingAssignment.length > 0) {
            // Update existing assignment with ALL actual columns
            await dbConn.query(
              `UPDATE mechanic_assignments 
               SET bay_number = ?,
                   ...
                   duration = ?
               WHERE id = ?`,
              [bayNumber, mechanicStatus, approvalStatus, approvedBy, approvedAt, scheduledStart, scheduledEnd, actualStartDatetime, actualEndDatetime, mechanicChallengeNotes, invoiceNotes, invoiceHours, actualHours, duration, existingAssignment[0].id]
            );
          } else {
            // Insert new assignment with ALL actual columns
            await dbConn.query(
              `INSERT INTO mechanic_assignments 
               (repair_item_id, mechanic_name, bay_number, status, approval_status, approved_by, approved_at, scheduled_start, scheduled_end, actual_start_datetime, actual_end_datetime, mechanic_challenge_notes, invoice_notes, invoice_hours, actual_hours, duration, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
              [repairItemId, mechanicName, bayNumber, mechanicStatus, approvalStatus, approvedBy, approvedAt, scheduledStart, scheduledEnd, actualStartDatetime, actualEndDatetime, mechanicChallengeNotes, invoiceNotes, invoiceHours, actualHours, duration]
            );
          }
        } else {
          // 🔴 FIX 3: No garage_mechanic_id - still check for existing by (repair_item_id, mechanic_name, scheduled_start) like PHP
          const [existingAssignment] = await dbConn.query(
            'SELECT id FROM mechanic_assignments WHERE repair_item_id = ? AND mechanic_name = ? AND ((scheduled_start IS NULL AND ? IS NULL) OR scheduled_start = ?)',
            [repairItemId, mechanicName, scheduledStart, scheduledStart]
          );
      ...
      // 🔴 FIX 2: Removed duplicate vm_logs INSERT (was appearing twice)
      await dbConn.query(
        `INSERT INTO vm_logs (ro_id, user_id, log_data, log_time) VALUES (?, ?, ?, NOW())`,
        [externalRoId, userId, `Garage Module API - Processed ${mechanics.length} mechanic assignment(s) for repair item ${repairItemId}`]
      );
    }
